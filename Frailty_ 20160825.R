frailty <- function(formula, data, logp, clusterid){
  ...
  fnscale <- -1
  fit <- optim(par=logp, fn=like, gr=gradientfunc, method="BFGS", hessian=FALSE, control=list(fnscale=fnscale))
  par <- fit$par
  ...
  #### Output ####
  out <- c(list(X=X, fit = fit, par = par, se = se, score = score, hessian = hessian, call = call,
